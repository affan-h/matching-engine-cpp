...
21. **Deterministic Bounded Shutdown Burst in Concurrency Tests**: Fixed `test_gateway_clean_graceful_shutdown_under_intense_traffic_burst` where unbounded client traffic loops placed `stop_traffic` after `gateway.stop()`, causing client threads to spin generating millions of orders while the consumer attempted to drain, resulting in 30-minute TSan delays; bounded burst size per thread (`MAX_BURST_PER_THREAD = 1000`) and stored `stop_traffic` prior to `gateway.stop()`, reducing TSan test duration from 1,807s to 2.4s.
